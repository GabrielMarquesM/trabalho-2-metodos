...
std::vector<double> Gauss_Seidel(std::vector<std::vector<double>> A,std::vector<double> id, int n, double epsilon, std::vector<double> &erro, int & iteracoes){
	...
	while(diff(x0_aux,x1)>=epsilon){
		for (int i = 0; i<n; i++){
			...
			for (int j=0; j<n; j++){
				if(i!=j){
					valor = valor + (aux.at(j)* (j<i? x1.at(j):x0.at(j)));
				}
			}
...
bool sassenfeld(std::vector<std::vector<double>> A, int n){
	...
	for(int i =0;i<n;i++){
		...
		for(int j = 0;j<n;j++){
			if(j!=i){
				numerador += fabs(linha.at(j))*beta.at(j);
			}
		}
		beta.at(i) = numerador/fabs(linha.at(i));
	}

	max_beta = *max_element(beta.begin(), beta.end());
	return max_beta < 1? true: false;
}